...
_pruning_target_cls = (nn.Linear, nn.Conv1d, nn.Conv2d, nn.Conv3d)


def _is_pruning_target(model, name, module):
    return name.startswith(model.bitstream_prefix) and not name.startswith(model.no_prune_prefix) and isinstance(module, _pruning_target_cls)
...
def set_pruning(model, ratio:float, weight:float):
    """
